...
# ╔═╡ 03f38afd-2b6f-4abd-ac97-aa2bcf121884
function demean(x)
	return x .- mean(x)
end
...
function matrix_to_vecvec(matrix)
	...
	vecvec = [matrix[i,:] for i in 1:nrow]
	return vecvec
end
...
begin
	...
	function quantize(color::AbstractRGB)
		return(RGB(quantize(red(color)), quantize(green(color)), quantize(blue(color))))
	end
...
# ╔═╡ ab4aabe7-94f2-4562-8e68-4bfdb8282983
# note sure why this doesn't work. had this problem throughout the homework with 
# loops. It's probably a thing with loops in Julia, something like no side effects 
# of loops. Would be so weird though. 
# begin
# 	L, C = size(philip)
# 	for l in L
# 		for c in C
# 			newphilip[l,c] = copy(invert(philip[l,c]))
# 		end
# 	end
# end
...
# ╔═╡ adfc1bad-23a6-4a26-a474-431b165a3454
begin
	function myclamp(x::Number, low, high)
		if x > high
			return high
		elseif x < low
			return low 
...
begin
	function noisify(x::Number, s)
		...
		s = sign*rand()*s
		toonoisy = x + s 
		noisy = myclamp(toonoisy, 0, 1)
		return noisy
	end
	
	function noisify(color::AbstractRGB, s)
		return(RGB(noisify(red(color), s), noisify(green(color), s),
			noisify(blue(color), s)))
	end
	
	function noisify(image::AbstractMatrix, s)
		return broadcast(noisify, image, (s))
	end
end
...
# ╔═╡ a253ebe0-78ad-4d71-9874-3d0524ba7023
broadcast(invert, philip)

# ╔═╡ 2de0edcb-8e75-440e-a399-2ce7731e3c27
noisify(philip[1,1], 0.8)
...
function blur_1D(v, l)
	v_blurred = deepcopy(v)
	for i in 1:n
		mysum = 0
		for j in (i-l):(i+l)
			mysum = mysum + extend(v, j)
		end
		v_blurred[i] = mysum/(2*l+1)
	end
	return v_blurred
end
...
# ╔═╡ 7ccac81a-1dbc-43b7-a12a-0e6d2ee11a23
@bind l_box Slider(1:1:4, show_value=true)

# ╔═╡ ca1ac5f4-ee1c-11ea-3d00-ff5268866f87
colored_line(v)

# ╔═╡ d4766dd4-815a-4334-936b-070e79a78974
...
function convolve_vector(v, k)
	l = length(k)÷2
	v_convolved = deepcopy(v)
	for i in 1:length(v)
		convolution = 0
		for j in -l:l
			convolution = convolution + extend(v, i+j) * k[l+j+1]
		end
		v_convolved[i] = convolution
	end
	return v_convolved
end

# ╔═╡ 5eea882c-ee13-11ea-0d56-af81ecd30a4a
colored_line(convolve_vector(v, [1,1,1,1,1]))
...
# ╔═╡ 15bfa37f-673b-4899-8a81-fa84cdbe30fa
test_test_convolution = let
	v_test = collect(1:10)
	convolve_vector(v_test, [1,1,1])
end
...
# ╔═╡ ebcedbd9-0eff-4344-a639-a02781f7f2cb


# ╔═╡ 56979174-25c8-443f-a6cf-26bd610d5e69
function gaussian(x)
	return (1/(2*π)^0.5)*exp(-(x)^2/2)
end

# ╔═╡ 1c8b4658-ee0c-11ea-2ede-9b9ed7d3125e
function gaussian_kernel(n)
	center = n÷2+1
    gaussian_values = zeros(n)
	total = 0 
	for i in 1:n
		gaussian_values[i] = gaussian(i-center)
		total = total + gaussian_values[i]
	end 
	for i in 1:n
		gaussian_values[i] = gaussian_values[i]/total
	end
...
function extend_mat(M::AbstractMatrix, i, j)
	L, K = size(M)
	if i in 1:L && j in 1:K
		return M[i,j]
	elseif !(i in 1:L) && j in 1:K
		return extend(M[:,j], i)
	elseif i in 1:L && !(j in 1:K)
		return extend(M[i,:], j)
	else 
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	L = size(K)[1]÷2
	C = size(K)[2]÷2
	convolved_M = deepcopy(M)
	for i=1:size(M)[1], j=1:size(M)[2]
		if eltype(M) <: AbstractRGB
			convolved_cell = RGB(0,0,0)
		else
			convolved_cell = 0
		end
		for l=-L:L,c=-C:C
			convolved_cell = convolved_cell + (K[l+L+1, c+C+1] * extend_mat(M, i+l, j+c))
		end
		convolved_M[i,j] = convolved_cell
	end
	return convolved_M
end
...
# ╔═╡ da2e92a8-c3cc-4784-a71b-de87e1e942f7
function gaussian_2D(x,y)
	return (1/(2*π))*exp(-(x^2 + y^2)/2)
end

# ╔═╡ 3cdde871-cde6-4b4c-a524-e4672931b93a
function gaussian_kernel_mat(n)
	mat = zeros(n,n)
	cols = zeros(n,n)
	rows = zeros(n,n)
	total = 0
	...
	for i in 1:n, j in 1:n
		cols[i,j] = j-(l+1)
		rows[i,j] = i-(l+1)
		mat[i,j] = gaussian_2D(i-(l+1), j-(l+1))
		total = total + mat[i,j]	
	end
	for i in 1:n, j in 1:n
...
function with_sobel_edge_detect(image)
	graystuff = channelview(image)
	G_x = [1 0 -1; 2 0 -2; 1 0 -1]
	G_y = transpose(G_x)
	G_x = convolve_image(graystuff, G_x)
	G_y = convolve_image(graystuff, G_y)
	gradient = (G_x.^2 + G_y.^2).^0.5
	return broadcast(Gray,gradient)
end
